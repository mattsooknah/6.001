    ...
    (make-handler
     ...
     (make-methods
      ...
      'DEL-THING   (lambda (thing)
		     ...
		     'DONE)
      'HAS-A ;; need this dummy method for smartphone methods to work (can be removed if smartphone is not used)
      (lambda (thing-type) '())
      'PEOPLE-AROUND ;; ditto above
      (lambda () '()))
     root-part)))
...
(define (person self name birthplace)
  (let ((mobile-thing-part (mobile-thing self name birthplace))
	...
	(ask thing 'CHANGE-LOCATION (ask self 'LOCATION)))
      ...
      (lambda (thing-type) 
        (filter 
         (lambda (x) (ask x 'IS-A thing-type)) 
         (ask self 'THINGS)))
      
      'HAS-A-THING-NAMED
      (lambda (thing-name) 
        (filter
         (lambda (x) (eq? thing-name (ask x 'NAME)))
         (ask self 'THINGS)))
...
(define (autonomous-person self name birthplace restlessness miserly)
  (let ((person-part (person self name birthplace))
        (initial-restlessness restlessness)
        (initial-miserly miserly))
    (make-handler
     'AUTONOMOUS-PERSON
     (make-methods
      'INSTALL
	...
	(ask our-clock 'ADD-CALLBACK
	     (create-clock-callback 'move-and-take-stuff self 'MOVE-AND-TAKE-STUFF))
   (ask our-clock 'ADD-CALLBACK
        (create-clock-callback 'require-food self 'REQUIRE-FOOD)))
      'MOVE-AND-TAKE-STUFF
      (lambda ()
	;; first move
	...
	'done-for-this-tick)
      'REQUIRE-FOOD
        (lambda ()
          (let ((my-food (filter (lambda (x) (ask x 'IS-A 'FOOD)) (ask self 'THINGS))))
          (if (and (not (zero? (current-time)))
                   (zero? (remainder (current-time) 10)))
              (if (pair? my-food) (ask (car my-food) 'eat)
                  (begin (ask self 'SUFFER 1)
                         (ask self 'CHANGE-RESTLESSNESS 1)
                         (ask self 'CHANGE-MISERLY 1))))))
      'RESET-RESTLESSNESS
      (lambda () (set! restlessness initial-restlessness))
      'RESET-MISERLY
      (lambda () (set! miserly initial-miserly))
      'CHANGE-RESTLESSNESS
      (lambda (new) (set! restlessness new))
      'CHANGE-MISERLY
      (lambda (new) (set! miserly new))
      'DIE
      (lambda ()
	(ask our-clock 'REMOVE-CALLBACK self 'move-and-take-stuff)
   (ask our-clock 'REMOVE-CALLBACK self 'require-food)
    ...
    (make-handler
     ...
     (make-methods
      ...
      (lambda () 
        (for-each 
         (lambda (room) 
           (if (not (null? (find-all room 'PERSON))) 
               (ask screen 'TELL-ROOM (ask self 'LOCATION) (list (ask room 'NAME) "is occupied."))))
         all-rooms))
      'WHO-WHERE
      (lambda () 
        (for-each
         (lambda (person) 
           (ask screen 'TELL-ROOM (ask self 'LOCATION) (list (ask person 'NAME) "is at" (ask (ask person 'LOCATION) 'NAME))))
         (all-people))))
     mobile-part)))

;;
;; smartphone (spec asks for PDA, but really?)
;;

(define (create-smartphone name location)
  (create-instance smartphone name location))

(define (smartphone self name location)
  (let ((mobile-part (mobile-thing self name location)))
    (make-handler
     'SMARTPHONE
     (make-methods
      'USE
      (lambda (hack target)
        (let ((user (ask self 'LOCATION)))
         (if (not (ask user 'IS-A 'PERSON)) 
             (ask self 'EMIT (list (ask user 'NAME) "You have to pick that up before you can use it"))
             (begin
               (ask screen 'TELL-ROOM (ask user 'LOCATION) (list (ask user 'NAME) "pulls out their smartphone"))
               (cond ((or (not hack) (null? hack)) (ask user 'SAY (list "I'm outta hacks")))
                     ((or (not target) (null? target)) (ask user 'SAY (list "Nothing left to hack")))
                     (else (ask hack 'USE user target)))))))
     'GOTCHA
     (lambda (target)
       (ask self 'USE (pick-random (ask (ask self 'LOCATION) 'HAS-A 'HACK)) target))
     'WHATS-THIS-DO
     (lambda (hack)
       (ask self 'USE hack (pick-random (ask (ask self 'LOCATION) 'PEOPLE-AROUND))))
     'LOOPEM
     (lambda (target)
       (for-each (lambda (x) (ask self 'USE x target)) (ask (ask self 'LOCATION) 'HAS-A 'HACK)))
     'HACK-TEH-WORLD
     (lambda (hack)
       (for-each (lambda (x) (ask self 'USE hack x)) (ask (ask self 'LOCATION) 'PEOPLE-AROUND))))
     mobile-part)))
  ...
  (let ((mobile-part (mobile-thing self name location)))
    (make-handler
     ...
     (make-methods
      ...
      (lambda ()
        (let ((user (ask self 'LOCATION)))
          (if (ask user 'IS-A 'PERSON)
              (begin
                (ask self 'EMIT (list (ask user 'NAME) "eats" (ask self 'NAME)))
                (ask user 'SAY (list "OM NOM NOM"))
                (ask user 'RESET-HEALTH)
                (if (ask user 'IS-A 'AUTONOMOUS-PERSON) 
                    (begin (ask user 'RESET-RESTLESSNESS)
                           (ask user 'RESET-MISERLY)))
                (ask self 'DESTROY))
              (ask self 'EMIT (list "You have to pick that up before you can use it")))))
      )
     mobile-part)))
