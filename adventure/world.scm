...
(define (create-world)
  ...
  (let ((10-250 (create-place '10-250))
        ...
        (forbes-cafe (create-place 'forbes-cafe))
    ...
    (can-go-both-ways stata-center 'east 'west student-street)
    (can-go-both-ways student-street 'north 'south forbes-cafe)
    ...
    (create-food 'cookie forbes-cafe)
    (create-food 'sushi forbes-cafe)
    (create-food 'chips forbes-cafe)
    (create-food 'subway-sandwich student-center)
    (create-food 'burger student-center)
    (create-food 'indian-food student-center)
    ...
    (let ((sp1
           (create-hack
            ...
            (lambda (hacker target)
              (if (not (ask target 'IS-A 'PERSON)) (ask target 'EMIT (list "nothing happens"))
              (begin (ask target 'EMIT (list (ask target 'NAME) "grows boils on their nose"))
                     (ask target 'HAVE-FIT))))))
          (sp2
           (create-hack
            'SLUG-HACK
          ...
          (sp3
           (create-hack 
            'FLEMING-CANNON 
            bowels
             "moved by Howe & Ser Moving Co." 
             (lambda (hacker target) 
               (ask target 'EMIT (list (ask target 'NAME)"goes BOOM!"))
           ...
           (sp4
            (create-hack
             ...
             bowels
             "it gives you wings"
             (lambda (hacker target-list)
               (if (not (pair? (car target-list))) (set! target-list (list target-list))) ; in case target is single object (which is represented as a list, hence the car)
               (for-each
                (lambda (target) 
                  (ask target 'EMIT (list (ask target 'NAME) "receives Red Bull"))
                  (if (ask target 'IS-A 'PERSON)
                      (begin (ask target 'RESET-HEALTH) (ask target 'SAY (list "I feel refreshed!")))
                      (if (= (random 3) 0) 
                          (begin (ask target 'EMIT (list (ask target 'NAME) "is broken"))
                                 (ask target 'DESTROY))
                          (ask target 'EMIT (list (ask target 'NAME) "is really wet"))))) 
                target-list))))
          )
      bowels)))
...
(for-each (lambda (hack) (clone-hack hack (ask me 'LOCATION))) (ask bowels-of-stata 'THINGS))

(create-mobile-thing 'HowToGAMIT (ask me 'LOCATION))
(ask me 'take (thing-named 'HowToGAMIT))
(ask me 'take (thing-named 'fleming-cannon))
(ask me 'has-a 'hack)
(ask me 'has-a-thing-named 'HowToGAMIT)
(ask me 'has-a 'fish)
(ask me 'has-a-thing-named 'wanda)



(create-GPS-tracker 'GARMIN (ask me 'LOCATION))
(ask me 'take (thing-named 'GARMIN))
(ask (thing-named 'garmin) 'OCCUPIED)
(ask (thing-named 'garmin) 'WHO-WHERE)



(create-person 'dummy1 (ask me 'LOCATION))
(create-person 'dummy2 (ask me 'LOCATION))
(create-smartphone 'iPhone (ask me 'LOCATION))
(ask (thing-named 'iPhone) 'GOTCHA (thing-named 'dummy1))
(ask me 'take (thing-named 'iPhone))
(ask (thing-named 'iPhone) 'GOTCHA (thing-named 'dummy1))
(ask me 'take (thing-named 'fleming-cannon))
(ask me 'take (thing-named 'slug-hack))
(ask (thing-named 'iPhone) 'GOTCHA (thing-named 'dummy1))
(ask (thing-named 'iPhone) 'WHATS-THIS-DO (thing-named 'slug-hack))
(ask (thing-named 'iPhone) 'LOOPEM (thing-named 'dummy2))
