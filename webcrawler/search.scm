...
(define (search-with-cycles initial-state goal? successors merge graph)
  (define (search-inner still-to-do history)
    (if (null? still-to-do)
        #f
        (let ((current (car still-to-do)))
          (if *search-debug*
              (begin (display "now at: ")
                     (display current)
                     (newline)))
          (cond ((contains? current history) 
                 (search-inner (cdr still-to-do) history))
                ((goal? current) 
                 #t) 
                (else (search-inner
                 (merge (successors graph current) (cdr still-to-do)) 
                 (cons current history)))))))
  (search-inner (list initial-state) nil))

(define (search-and-apply initial-state procedure goal? successors merge graph)
  (define (already-visited? node history)
    (pair? (memq node history)))
  (define (search-inner still-to-do history)
    (if (null? still-to-do)
        #f
        (let ((current (car still-to-do)))
          (if *search-debug*
              (begin (display "now at: ")
                     (display current)
                     (newline)))
          (if (already-visited? current history) 
              (search-inner (cdr still-to-do) history)
              (begin (procedure current)
                (if (goal? current) #t 
                 (search-inner
                  (merge (successors graph current) (cdr still-to-do)) 
                  (cons current history))))))))
...
(define (contains? elt list)
  (cond ((null? list) #f)
        ((eq? elt (car list)) #t)
        (else (contains? elt (cdr list)))))
...
(define (BFS-active start proc goal? graph)
  (search-and-apply
   start
   proc
   goal?
...
(define (no-goal node) #f)

;; the only difference between DFS and BFS is where we put the current node's children
;; in the search queue. by putting them at the beginning, we get DFS. by putting them 
;; at the end, we get BFS.
...
;(BFS-active 'a 
;            (lambda (node) (display "Hello world!") (newline))
;            (lambda (node) (eq? node 'magic)) 
;            test-cycle)
...
(define (add-document-to-index! index web url)
  (define (text-iter index text url)
    (if (pair? text)
        (begin 
          (add-to-index! index (car text) url)
          (text-iter index (cdr text) url))))
  (text-iter index (find-URL-text web url) url))

;; make-web-index: Web, URL -> (symbol -> list<URL>)
;; make-web-index creates an index from an existing web and starting URL
;; and returns a procedure for looking up URLs that contain a given symbol/word
(define (make-web-index web url)
  (define web-index (make-index))
  (BFS-active url (lambda (doc) (add-document-to-index! web-index web doc)) no-goal web)
  (lambda (word) (find-in-index web-index word)))

;; search-any: Web, URL, symbol -> URL
;; indexes web in real time, returning *first* URL containing desired word/symbol
(define (search-any web start-url word)
  (define matching-url #f)
  (BFS start-url 
    (lambda (url) 
      (if (contains? word (find-URL-text web url))
          (begin (set! matching-url url) #t) #f))
    web)
  matching-url)

;; search-all: Web, URL, symbol -> list<URL>
;; indexes web in real time, returning *all* URLs containing desired word/symbol
(define (search-all web start-url word)
  (define matching-url-list nil)
  (BFS start-url 
    (lambda (url) 
      (if (contains? word (find-URL-text web url))
          (set! matching-url-list (cons url matching-url-list)))
      #f)
    web)
  (if (null? matching-url-list) #f matching-url-list))

;;;------------------------------------------------------------
;;; Optimized Index
;;;
;;; For our existing index abstraction, search takes O(n) time.
;;; Using vectors instead of lists, we can search in O(log n) time.
;;;
;;; Key = symbol
;;; Val = symbol

;; insertion-sort: list<A>, (A,A -> boolean) -> list<A>
;; because SICP Scheme doesn't have sort...
(define (insertion-sort lst pred?)
  (define (insert sorted-left elt sorted-right unsorted)
    (if (or (null? sorted-right) (pred? elt (car sorted-right)))
        (iterate (append sorted-left (list elt) sorted-right) unsorted)
        (insert (append sorted-left (list (car sorted-right))) elt (cdr sorted-right) unsorted)))
  (define (iterate sorted unsorted)
    (if (null? unsorted) 
        sorted
        (insert nil (car unsorted) sorted (cdr unsorted))))
  (iterate nil lst))

;; optimize-index: Index -> OptIndex
(define (optimize-index ind)
  (list 'optimized-index (list->vector (insertion-sort (cdr ind) key-sorted?))))

(define (key-sorted? entry1 entry2)
  (string<? (symbol->string (car entry1))
            (symbol->string (car entry2))))

(define (binary-search vect target precedes? result)
  (define (iterate lower upper)
    (if (> lower upper) #f
        (let* ((midpoint (round (/ (+ lower upper) 2)))
               (midval (vector-ref vect midpoint)))
          (cond ((precedes? target midval)
                 (iterate lower (- midpoint 1)))
                ((precedes? midval target)
                 (iterate (+ midpoint 1) upper))
                (else (result midval))))))
  (iterate 0 (- (vector-length vect) 1)))

;; find-entry-in-optimized-index: OptIndex, Key -> list<Val>
;; performs binary search for some key k in O(log n) time
(define (find-in-optimized-index optind k)
  (binary-search (cadr optind) (list k) key-sorted? (lambda (entry) (cadr entry))))
          
;(define test-index (make-index))
;(add-to-index! test-index 'b-key 'baz)
;(add-to-index! test-index 'b-key 'bar)
;(add-to-index! test-index 'c-key 'qux)
;(add-to-index! test-index 'a-key 'foo)
;(define opt-test-index (optimize-index test-index))
;opt-test-index
;(find-in-optimized-index opt-test-index 'a-key)
;(find-in-optimized-index opt-test-index 'b-key)
;(find-in-optimized-index opt-test-index 'c-key)
